...
class AsciiTable
{
	...
	*/
	public function makeTable($array, $title = '', $return = false)
	{
	...
	*/
	public function breakTable($table)
	{
	...
	*/
	public function scrapeTable($table, $key, $value = null)
	{
		...
		foreach ($array as $row_num => $row) {
			foreach ($row as $col => $value) {
				...
				if (!isset($this->col_widths[$col])) {
					$this->col_widths[$col] = mb_strlen($col);
				}

				// Check if this column is the longest, if so update $col_width
				else if ($this->col_widths[$col] < mb_strlen($value)) {
					$this->col_widths[$col] = mb_strlen($value);
				}
			}
		}
	}
	...
	*/
	private function makeTitle($title)
	{
		...
		$left_padding = floor(($this->table_width - mb_strlen($title)) / 2);
	...
	*/
	private function makeHeaders()
	{
		...
		foreach ($this->col_widths as $col => $length) {
			...
			if (mb_strlen($col) < $length) {
				$row .= str_repeat(' ', $length - mb_strlen($col));
			}
	...
	private function makeRows($array)
	{
		...
		foreach ($array as $n => $data) {
			...
			foreach ($data as $col => $value) {
				...
				if (mb_strlen($value) < $this->col_widths[$col]) {
					$rows .= str_repeat(' ', $this->col_widths[$col] - mb_strlen($value));
				}
