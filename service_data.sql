SELECT f.film_name AS "Фильм",
   CASE
       WHEN v.date_val = CURRENT_DATE THEN a.attr_name::text
       ELSE NULL::text
       END AS "Задачи на сегодня",
   CASE
       WHEN v.date_val = (CURRENT_DATE + '20 days'::interval day) THEN a.attr_name::text
       ELSE NULL::text
...
WHERE at.type_name::text = 'служебные даты'::text
   AND (v.date_val = CURRENT_DATE OR v.date_val = (CURRENT_DATE + '20 days'::interval day));
